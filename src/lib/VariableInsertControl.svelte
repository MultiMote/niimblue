  ...
  <div class="dropdown-menu px-2">
    <div class="d-flex gap-1 flex-wrap">
      <button class="btn btn-secondary btn-sm" on:click={() => insertDateTime()}
        ><FaIcon icon="calendar" /> {$tr("params.variables.insert.datetime", "Datetime")}</button
      >
      <button class="btn btn-secondary btn-sm" on:click={() => insertDateTime("YYYY-MM-DD")}
        ><FaIcon icon="calendar" /> {$tr("params.variables.insert.date", "Date")}</button
      >
      <button class="btn btn-secondary btn-sm" on:click={() => insertDateTime("HH:mm:ss")}
        ><FaIcon icon="clock" /> {$tr("params.variables.insert.time", "Time")}</button
      >
    </div>
  </div>
</div>
