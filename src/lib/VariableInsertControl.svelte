...
<div class="btn-group " role="group" title={$tr("params.variables.insert")}>
  ...
  <div class="dropdown-menu px-2">
    <div class="d-flex gap-1 flex-wrap">
      <button class="btn btn-secondary" on:click={() => insertDateTime()}>
        <MdIcon icon="calendar_today" />
        {$tr("params.variables.insert.datetime")}
      </button>
      <button class="btn btn-secondary" on:click={() => insertDateTime("YYYY-MM-DD")}>
        <MdIcon icon="calendar_today" />
        {$tr("params.variables.insert.date")}
      </button>
      <button class="btn btn-secondary" on:click={() => insertDateTime("HH:mm:ss")}>
